...
main(int argc, char **argv)
{
    ...
    for(vector<char *>::iterator it = userProg.begin();it!=userProg.end();++it){
		char *ch = *it;
                int len = strlen(ch);
                string s="";
		...
		t ->Fork((VoidFunctionPtr) RunUserProg, (void *) *it);
	} 
